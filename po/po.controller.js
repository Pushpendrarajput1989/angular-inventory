...
poController.$inject = ['$scope', '$http', '$location', 'baseUrl', '$mdDialog', '$mdMedia', 'growl', '$window', 'downloadOrderTemplateUrl', 'Upload','PagerService'];

function poController($scope, $http, $location, baseUrl, $mdDialog, $mdMedia, growl, $window, downloadOrderTemplateUrl, Upload,PagerService) {
    ...
    $scope.listOfOrders = function(tabsValue, start, action) {
        ...
        if ($scope.defaultTab == 'all')
            orderListUrl += "?start=" + start + "&size=5&sort=tablePurchaseOrderDate&direction=desc";

        if ($scope.defaultTab != 'all')
            //orderListUrl += "?&state=" +  tabsValue;
            orderListUrl += "?start=" + start + "&size=5&sort=tablePurchaseOrderDate&direction=desc&state=" + tabsValue;
...
//Test Abhinit
        ...
        console.log(JSON.parse($scope.singleorderData.payment));
        ...
        if($scope.products.length == 0){
            ...
            var postData = {
                ...
                "tablePurchaseOrderPaymentType": JSON.parse($scope.singleorderData.payment),
                ...
                "tableVendor": JSON.parse($scope.singleorderData.vendorData),
                "tableWarehouseDetails": JSON.parse($scope.singleorderData.wareHousesData)
            }
    ...
    $scope.getPriceOfProduct = function(skuId, saleChannelId,quantity) {
        $http({
            method: 'GET',
            url: baseUrl + '/omsservices/webapi/vendors/vendorsystemskumap/sku/' + skuId + '/vendor/' + saleChannelId +'/skuquantity/'+quantity+ '/price',
            headers: {
                'Content-Type': 'application/json'
            }
        }).success(function(res) {
...
//    ================================== table row expnsion ================================= //
    ...
    $scope.stateTrials = function(saleordskus) {
        ...
        for (var i = 0; i < saleordskus.length; i++) {
            ...
            $scope.StateArray.push(saleordskus[i].tablePurchaseOrderSkuStateType.tablePurchaseOrderSkiuStateTypeString);
            ...
            if (trials.length < 4) {
                for (var j = 0; j < trials.length; j++) {
                    console.log(trials[j].tablePurchaseOrderSkuStateType.tablePurchaseOrderSkiuStateTypeString);
                    $scope.trialsDataArray.push(trials[j].tablePurchaseOrderSkuStateType.tablePurchaseOrderSkiuStateTypeString);
    ...
    $scope.openInfoBox = function(ev, stateTrials) {
        ...
        for (var i = 0; i < stateTrials.length; i++) {
            ...
            var fulldate = stateTrials[i].tablePurchaseOrderSkuStateTrailTimestamp[2] + "/" + stateTrials[i].tablePurchaseOrderSkuStateTrailTimestamp[1] + "/" + stateTrials[i].tablePurchaseOrderSkuStateTrailTimestamp[0];
            if (i < a) {
                $scope.steps.push({
                    title: stateTrials[i].tablePurchaseOrderSkuStateType.tablePurchaseOrderSkiuStateTypeString,
                    active: true,
...
//=============================================== Searching po ========================== //
    ...
    $scope.listOfStatesCount = function(tabsValue, page, action) {
        ...
        var newCountUrl = baseUrl + "/omsservices/webapi/purchase/order/filtercount?state=new";
        var processCountUrl = baseUrl + "/omsservices/webapi/purchase/order/filtercount?state=inprocess";
        var holdCountUrl = baseUrl + "/omsservices/webapi/purchase/order/filtercount?state=intransit";
        var returnCountUrl = baseUrl + "/omsservices/webapi/purchase/order/filtercount?state=grn";
        var cancelledCountUrl = baseUrl + "/omsservices/webapi/purchase/order/filtercount?state=cancelled";
        ...
        var allCountUrl = baseUrl + "/omsservices/webapi/purchase/order/filtercount?";
